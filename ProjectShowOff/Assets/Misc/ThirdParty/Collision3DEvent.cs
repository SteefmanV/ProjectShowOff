...
[Serializable]
public class Collision3DEvent : ScriptableObject
{
    private Type type;
    ...
    public void HandleEvent(UnityEvent unityEvent, Collider col)
    {
    ...
    private void InvokeByType(UnityEvent unityEvent, Collider col)
    {
...
#if UNITY_EDITOR
[CustomEditor(typeof(Collision3DEvent),true)]
public class EventTriggerEditor : Editor
{
    private Collision3DEvent trigger;
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (trigger == null)
            trigger = target as Collision3DEvent;
        ...
        {
            trigger.collisionType = (CollisionTypes) EditorGUILayout.EnumPopup("Type", trigger.collisionType);
    ...
    private void SelectCollisionByType()
    {
        ...
        {
            if (allTypes[i].IsSubclassOf(typeof(MonoBehaviour)) && !allTypes[i].FullName.Contains("NewtonVR") && !allTypes[i].FullName.Contains("Valve") && !allTypes[i].FullName.Contains("SteamVR"))
            {
                types.Add(allTypes[i]);
                string name = allTypes[i].FullName;
